...
<<<<<<< HEAD
# Versi Streamlit 3.0 (2024) - FINAL SYNTAX FIX QUADRUPLE VERIFIED
=======
# Versi Streamlit 2.7 (2024) - Final Syntax Fix VERIFIED
>>>>>>> 8c7913c7b5149112366845d47a661695218db697
...
>>>>>>> 8c7913c7b5149112366845d47a661695218db697
        with suppress(locale.Error):
            ...
            if '.' in test_format or ',' in test_format:
                # print(f"Locale set to: {loc}") # Debugging
                return # Locale supports grouping, exit
    # print("Locale with grouping not found, using fallback.") # Debugging
...
def format_currency(amount):
    ...
    try:
        amount = float(amount)
<<<<<<< HEAD
        # Ensure locale is set before attempting to format
        setup_locale()
        # Use locale formatting if available
        return locale.currency(amount, symbol="Rp ", grouping=True, international=False)
    except (ValueError, TypeError):
        return "Rp 0" # Handle non-numeric input
    except locale.Error:
        # Fallback formatting if locale setting fails or doesn't support grouping
        try:
            # Simple fallback: comma for thousands, no decimals
            return f"Rp {amount:,.0f}".replace(",", ".")
        except:
             return "Rp 0" # Final fallback

def calculate_roi(investment, annual_gain, years):
    """Hitung ROI dalam persen untuk X tahun"""
    if investment <= 0: # Check for zero or negative investment
        return float("inf") # Or return "N/A" or None, depending on desired output
    total_gain = annual_gain * years
    try:
        # ROI formula: ((Total Gain - Investment) / Investment) * 100
        return ((total_gain - investment) / abs(investment)) * 100
    except ZeroDivisionError: # Should be caught by the check above
=======
        return locale.currency(amount, symbol="Rp ", grouping=True, international=False)
    except (ValueError, TypeError):
        return "Rp 0"
...
<<<<<<< HEAD
    ...
    try:
        ...
        months = 60
        # Safely get data using .get() with default 0
        ...
        # Ensure figure is closed if created but failed
    ...
    try:
        ...
        for bar in bars:
            yval = bar.get_height()
            # **CRITICAL FIX AREA: Ensure NO backslashes before quotes**
            # Using standard single quotes for va and ha parameters.
...
<<<<<<< HEAD
    ...
    roi_1_year_val = report_data.get("roi_1_year", float('inf')) # Default to inf if not calculated
    roi_5_year_val = report_data.get("roi_5_year", float('inf')) # Default to inf if not calculated
    payback_period_val = report_data.get("payback_period", float('inf'))

    # Table for main results
    col_width = pdf.w / 2 - pdf.l_margin - 1 # Adjust column width
    line_height = 7
    ...
    pdf.ln(5)
    chart_paths = []
    if figs: # Check if figs list is not empty
        for i, fig in enumerate(figs):
            if fig is None: continue # Skip if a figure failed to generate
            try:
                chart_path = f"/tmp/chart_{i}.png"
                # Use a higher DPI for better quality in PDF
                ...
                # Ensure figure is closed even if embedding fails
    ...
    try:
        # Use 'S' to output as bytes string
...
<<<<<<< HEAD
    ...
    with st.sidebar:
        ...
        # Check if all consultant fields are filled
        ...
        with col2_op:
            ...
            # Ensure slider value is correctly formatted
        ...
        st.markdown("---")
        # Disable button if consultant info is missing
    ...
    if hitung_roi:
        # Re-check consultant info (button should prevent this, but safety first)
        if not consultant_info_filled:
        ...
        with st.spinner("⏳ Menghitung ROI dan menyiapkan laporan..."): 
            ...
            # Ensure total monthly savings is calculated correctly
            ...
            # Use .get() for robustness when creating the dictionary
            report_data = {
                ...
                "noshow_rate_before": noshow_rate * 100, # Store as percentage
                "avg_salary": avg_salary,
                "revenue_per_appointment": revenue_per_appointment,
                "staff_reduction_pct": staff_reduction * 100, # Store as percentage
                "noshow_reduction_pct": noshow_reduction * 100, # Store as percentage
            ...
            figs = [] # Initialize figs list
            try:
                figs = generate_charts(report_data)
                if figs:
                    for fig in figs:
                        if fig: # Check if figure object is valid
                           st.pyplot(fig)
                else:
                ...
                # Ensure figs is an empty list if generation fails
                figs = [] 
            ...
            try:
                with st.spinner("Membuat laporan PDF..."): 
                    # Pass figs list (might be empty if charts failed)
                ...
                if pdf_content:
                    ...
                    st.error("Gagal membuat konten PDF (fungsi generate_pdf_report mengembalikan None).")

            except Exception as pdf_gen_err:
                st.error(f"Terjadi kesalahan saat membuat PDF: {pdf_gen_err}")
                st.code(traceback.format_exc())
                pdf_content = None # Ensure pdf_content is None if generation fails
            ...
            # Proceed only if PDF was generated and credentials exist
                ...
                if drive_service:
                    with st.spinner("Mengunggah PDF ke Google Drive..."): 
                        ...
                        if pdf_link:
                            ...
                            report_data["pdf_link"] = pdf_link # Update link in data for sheet
                        else:
                            st.error("Gagal mengunggah PDF ke Google Drive. Periksa ID Folder Drive dan izin Service Account.")
                else:
                ...
                if gc:
                    with st.spinner("Menyimpan data ke Google Sheet..."): 
                        # Prepare row data (ensure order matches sheet columns)
                        sheet_row = [
                            ...
                            report_data.get("pdf_link", "") # Use updated link
                        ]
                        # Convert all items to string, handle None/inf safely
                        sheet_row_str = []
                        ...
                        if sheet_success:
                            ...
                            st.error("Gagal menyimpan data ke Google Sheet. Periksa ID Sheet, Nama Sheet, dan izin Service Account.")
                else:
                ...
                # Specific warnings handled within upload/append blocks

            elif not pdf_content:
                 st.error("Sinkronisasi Google dilewati karena PDF gagal dibuat.")
            # No else needed here, handled by creds check
...
if __name__ == "__main__":
    ...
    if "google_credentials" not in st.secrets:
        # Display warning prominently if secrets are missing
